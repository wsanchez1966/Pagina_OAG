...
function createProductCard(item, user) {
  ...
  $brand.textContent = item.brand ? item.brand : 'Sin marca'
  $info.appendChild($brand)

  // Verificar si es producto de marcas públicas para mostrar siempre precio y stock
  const publicBrands = ['camel bridge', 'safeway', 'enduro', 'ypf röd', 'maya']
  const isPublicBrand = item.brand && publicBrands.includes(item.brand.toLowerCase())

  const $price = document.createElement('div')
  if (user || isPublicBrand) $info.appendChild($price)
  ...
  if (user || isPublicBrand) $info.appendChild($stock)

  const $stockQuantity = document.createElement('span')

  // Verificar si se debe mostrar indicador de color o cantidad numérica
  const userObj = user ? JSON.parse(user) : null
  const shouldShowColorIndicator = !userObj || userObj.role === 3 // Sin login o cliente nivel 3

  if (shouldShowColorIndicator && item.stockColor) {
    // Mostrar indicador de color para clientes o usuarios sin login
    const $colorIndicator = document.createElement('span')
    $colorIndicator.classList = 'stock-indicator'

    let backgroundColor = '#999' // Color por defecto
    let stockLabel = 'Sin información'

    switch (item.stockColor.toUpperCase()) {
      case 'V':
    ...
    $colorIndicator.style.backgroundColor = backgroundColor
    $colorIndicator.setAttribute('title', stockLabel)
    $stockQuantity.appendChild($colorIndicator)
  } else {
    // Mostrar cantidad numérica para admin y vendedores
